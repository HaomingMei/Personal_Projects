...
int32_t BSP_SDRAM_Init(uint32_t Instance)
{
  ...
  else
  {
      ...
      // TODO: Update the Pins as Necessary
      SDRAM_MspInit(&hsdram[0]); // Sets the Necessary pins for FMC
#endif /* USE_HAL_SDRAM_REGISTER_CALLBACKS */
    // TODO: Update the Pin Config in the ioc file as necessary
    if(MX_SDRAM_Init(&hsdram[0]) != HAL_OK) // Sets up the FMC (timing & control)
    {
    ...
    else
    {
      ...
      // Sets up the SDRAM Configuration, so it knows how to respond to the MCU
      // This is done by configuring its registers
